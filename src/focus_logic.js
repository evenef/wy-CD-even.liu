...
(function(){
	...
	function Page(){
		...
		this.move = function(direction){
			...
			this.moveDirection(direction); //方向键事件
		...
		this.moveDirectionEvent = undefined;		//方向键事件0，1，2，3   上下左右
		this.moveDirection = function(direction){
			if((!!this.areas[this.curAreaIndex].moveDirection(direction)) && this.moveDirectionEvent!=undefined){
				this.moveDirectionEvent(direction);
			};
		};
		...
		this.setCurrentFocus = function(_curAreaIndex,_curDomIndex){
			...
			if(_curDomIndex != undefined){
				...
				if(!!flag){
					this.areas[_curAreaIndex].setCurrentFocus("","inside");
				}else{
	...
	function Area(row,colum,doms,areaDirections,curDomIndex){
		...
		this.preDomIndex = this.curDomIndex;	//当前焦点的上一个焦点
		...
		this.objFocus = undefined;	//设置焦点移动的滑动效果["menu_top_focus",[20,125],[101,0],10)]个参数分别表示焦点框id,横向平移的各项属性[每次移动步长,初始left属性值],纵向移动的各项属性[步长，初始top值],移动到下个焦点的总时长
		...
		this.moveDirectionEvent = undefined;		//方向键事件0，1，2，3   上下左右
		this.moveDirection = function(direction){
			if(!!this.doms[this.curDomIndex].moveDirection(direction)){
				if(this.moveDirectionEvent != undefined){
					return this.moveDirectionEvent(direction);
				}else{
					return true;
				};
			}else{
	...
	//有盒子不支持onkeydown 768虚拟事件的可使用document.onkeypress代替
/*	document.onkeypress = keyPress;
	...
	function keyDown(event) {
		...
		switch (key_code) {
			...
			case 290://KEY_MENU
				doRedirectKey("menu");
			break;
            case 272://UT STB KEY_MENU
                clearAllCookie();
            break;
			case 768:
				try{
					if(goUtility!=undefined){
						goUtility();
					};
				}catch (e){};
			break;
	...
	*/
	...
	/**
	* @description 离开页面时保存页面信息
	* @isNeedSaveUrl 值为Array:[true(false),true(false)],第一个值表示是否保存url,第二个值表示是否保存url所带的参数
	* 				 值为Boolean:是否保存url,如果为true则保存url的参数
	* 				 值为String:直接保存String的值
	* @isNeedSaveFocus boolean:是否保存焦点
	* 如果两个参数都为undefined则默认保存url(包括参数)和焦点
	*/
	function savePageInfo(isNeedSaveUrl,isNeedSaveFocus){
		//url&focus
		if(isNeedSaveUrl==undefined && isNeedSaveFocus==undefined){
			addUrl(true);
			saveFocus();
			return;
		};
		//url
		if(isNeedSaveUrl.constructor == Array){
			if(isNeedSaveUrl[0]){
				if(isNeedSaveUrl[1]==false){
					addUrl(false);
				}else{
					addUrl(true);
				};
			};
		}else if(isNeedSaveUrl.constructor == Boolean){
			if(isNeedSaveUrl){
				addUrl(true);
			};
		}else if(isNeedSaveUrl.constructor == String){
			addUrl(isNeedSaveUrl);
		};
		//focus
		if(!!isNeedSaveFocus || isNeedSaveFocus==undefined){
			saveFocus();
		};
	};
	//将当前的页面加入Url队列
	//"url;/SXTV/view/pal/cn/260/default/portal.html;/SXTV/view/pal/cn/260/default/tvod.html"
	function addUrl(isNeedParam){
		var curSavedUrl = getCookie("_AUTOCEPHALY_URL_LIST");
		if(curSavedUrl==null || curSavedUrl==""){
			curSavedUrl = "url";
		};
		if(isNeedParam.constructor == Boolean){
			if(isNeedParam){
				curSavedUrl += ";"+location.pathname+location.search;
			}else{
				curSavedUrl += ";"+location.pathname
			};
		}else if(isNeedParam.constructor == String){
			curSavedUrl += ";"+isNeedParam; 
		};
		setCookie("_AUTOCEPHALY_URL_LIST",curSavedUrl);
	};
	//获取Url队列里最后一个Url,参数isPersistUrl获取url之后是否保留,值为Boolean类型,true为保留,false/undefined为删除
	function getLastUrl(isPersistUrl){
		var curSavedUrl = getCookie("_AUTOCEPHALY_URL_LIST");
		if(curSavedUrl!=null){
			var urlList = curSavedUrl.split(";");
			var lastUrl = urlList[urlList.length-1];
			if(!isPersistUrl){
				curSavedUrl = curSavedUrl.substring(0,curSavedUrl.lastIndexOf(";"));
				setCookie("_AUTOCEPHALY_URL_LIST",curSavedUrl);
			};
			delCookie("IS_PRESS_OK_IN");
			return lastUrl=="url"?"":lastUrl;
		}else{
			return "";
		};
	};
	...
	function saveFocus(){
		...
		var focusKey = "AUTOCEPHALY_"+path_name.substring(path_name.lastIndexOf("/")+1,path_name.indexOf("."));
		var pageLength = focus_logic.page.areas.length;
		var curPageFocus = ""+focus_logic.page.curAreaIndex;
		for(var i=0;i<pageLength;i++){
			curPageFocus += ","+focus_logic.page.areas[i].curDomIndex;
			curPageFocus += ","+focus_logic.page.areas[i].pageNumber;
		};
		setCookie("IS_PRESS_OK_IN","1");	//此参数用于区别通过点击ok键进入页面还是返回进入页面的,值为1则是通过ok键进入，值为null则为返回进入
		setCookie(focusKey,curPageFocus);
	};
	//获取Focus,参数isPersistFocus获取Focus之后是否保留,值为Boolean类型,true为保留,false/undefined为删除
	//参数defaultFocus：设置默认焦点设置方式如[1,0,1]表示区域1的0号焦点第一页,如要设置多个区域则以二维数组的形式[[1,0,1],[2,1,1]]数组中第一个为默认焦点所在的区域
	function getFocus(defaultFocus,isPersistFocus){
		var IS_PRESS_OK_IN = getCookie("IS_PRESS_OK_IN");
		var path_name = location.pathname;
		var focusKey = "AUTOCEPHALY_"+path_name.substring(path_name.lastIndexOf("/")+1,path_name.indexOf("."));
		var focusValue = getCookie(focusKey);
		//if(!isPersistFocus && IS_PRESS_OK_IN==null){
        if(!isPersistFocus){
			delCookie(focusKey);
		};
		var focusObj = new Object();
		focusObj["curAreaIndex"] = 0;
		focusObj["areaInfo"] = new Array();
		//if(focusValue!=null && IS_PRESS_OK_IN==null){
        if(focusValue!=null){
			var valueList = focusValue.split(",");
			var valueListLength = valueList.length;
			focusObj["curAreaIndex"] = parseInt(valueList[0]);
			var areaFocusObj = new Object();
			for(var i=1;i<valueListLength;i++){
				if(i%2 == 1){
					areaFocusObj = new Object();
					areaFocusObj["curDomIndex"] = parseInt(valueList[i]);
				}else if(i%2 == 0){
					areaFocusObj["pageNumber"] = parseInt(valueList[i]);
					focusObj["areaInfo"].push(areaFocusObj);
				};
			};
		}else{
			var pageLength = focus_logic.page.areas.length;
			for(var i=0;i<pageLength;i++){
				var areaFocusObj = new Object();
				areaFocusObj["curDomIndex"] = 0;
				areaFocusObj["pageNumber"] = 1;
				focusObj["areaInfo"].push(areaFocusObj);
			};
			if(defaultFocus!=undefined && defaultFocus.constructor==Array && defaultFocus.length>0 && defaultFocus[0].constructor==Array){
				focusObj["curAreaIndex"] = defaultFocus[0][0];
				for(var i=0;i<defaultFocus.length;i++){
					focusObj.areaInfo[defaultFocus[i][0]].curDomIndex = defaultFocus[i][1];
					focusObj.areaInfo[defaultFocus[i][0]].pageNumber = defaultFocus[i][2];
				};
			}else if(defaultFocus!=undefined && defaultFocus.constructor==Array){
				focusObj["curAreaIndex"] = defaultFocus[0];
				focusObj.areaInfo[defaultFocus[0]].curDomIndex = defaultFocus[1];
				focusObj.areaInfo[defaultFocus[0]].pageNumber = defaultFocus[2];
			};
		};
		return focusObj;
	};
	...
	function setCookie(name,value){
		...
		try{
			var stb_type = Authentication.CTCGetConfig("STBType");	//针对华为EC1308H盒子设置时间大于当前时间时不能保存cookie的问题做的适配
    ...
    function clearAllCookie(){
        if(document.cookie.match("AUTOCEPHALY")){
            var _arr = document.cookie.match("AUTOCEPHALY")['input'];
            _arr = _arr.split(";");
            for(var _i=0;_i<_arr.length;_i++){
                var _tmp = _arr[_i].split("=");
                var _cookie_name = _tmp[0];
                delCookie(_cookie_name.trim());
            };
        }; 
    };
	...
	window.focus_logic["savePageInfo"] = savePageInfo;
	window.focus_logic["getLastUrl"] = getLastUrl;
	window.focus_logic["getFocus"] = getFocus;
	window.focus_logic["getParameter"] = getParameter;
	window.focus_logic["getCookie"] = getCookie;
	window.focus_logic["setCookie"] = setCookie;
	window.focus_logic["delCookie"] = delCookie;
    window.focus_logic["clearAllCookie"] = clearAllCookie;
