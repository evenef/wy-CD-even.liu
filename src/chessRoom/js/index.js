...
function initData(obj) {
	data = {
		...
		spId: '40006',
		PRODUCTID: obj.PRODUCTID || "16021215165842000003",
		// contentCode: obj.contentCode || '1632121216155963739019',
		contentCode: obj.contentCode || '1632121216140011138725',
...
window.onload = function () {
	...
	initData(obj) //初始化数据
	toSendPage('page', '家娱棋牌屋')//统计上报页面信息
	auth()
	...
	if (data.UserID){
		switch (event.keyCode) {
			...
			if(getClass("payBtn_cl").length){
				toSendPage('toOrder', '家娱棋牌屋', '跳转订购页')
				toPayFnc()
			}else if(getClass("button_cl")[0].id == "0-0" && !currentPage){
				toSendPage('toActivityPage', '家娱棋牌屋', '跳转活动页：玩游戏拿大奖')
...
function initList(arr) {
	...
	if (currentPage < 1) { //首页菜单
		arr.forEach(function (item, index) {
				if (!index) {
					list.innerHTML += '<div class="buttonItem button_cl" style="background-image:url(./img/firstImg.png);background-size:184px 332px;background-position:center -3px;"></div>'
				...
				addButtonItem(item, middle, !(index % 2) ? "./img/product_green_bg.png" : "")
				return
...
function startActivity(gameId, userId) {
	...
	switch(true){
		...
		case /chessRoom/.test(location.href): referPageName = '家娱棋牌屋';referPageID = 'JiaYuQiPaiWu';break;
		case /ChangWanTing/.test(location.href): referPageName = '畅玩厅';referPageID = 'ChangWanTing';break;
	}
...
function boxMove(box, eleAfter) {
	...
	w = (wA - wBox) / 1,
	h = (hA - hBox) / 1,
	xA = eleAfter.offsetLeft + (eleAfter.id === "payBtn" ? 0 : listLeft) + ((eleAfter.id === "0-0") ? 0 : middleLeft) - (eleAfter.id === "payBtn" ? middleLeft : 0),
